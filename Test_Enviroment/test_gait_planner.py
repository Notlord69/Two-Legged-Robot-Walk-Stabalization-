...
import numpy as np
import pytest
from shared_state import shared_state, Siclo1State, ContactState, MissionState, StepPhase
...
def _reset_for_planner(mission_state=MissionState.WALK):
    ...
    # FSM fields required by 5-state gait_planner
...
def test_step_count_increments_at_phase_end():
    """When PLACE completes (contact confirmed + settled), step_count increments."""
    from gait_planner import update_gait_planner, SWING_DURATION
    _reset_for_planner()
    # Drive into PLACE phase with contact already confirmed and foot settled
...
def test_swing_side_swaps_after_step():
    """active_swing_side flips left↔right when PLACE completes."""
    from gait_planner import update_gait_planner, SWING_DURATION
    _reset_for_planner()
    shared_state.active_swing_side = "left"
    shared_state.step_phase  = StepPhase.PLACE
